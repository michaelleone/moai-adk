---
description: "Full text of moai-mcp-tools.md — the always-loaded file is a digest; this companion carries the complete doctrine verbatim"
paths: "**/moai-mcp-tools.md"
---

> **Full-text companion of `moai-mcp-tools.md`.** The always-loaded `moai-mcp-tools.md` is a digest: it restates every binding rule in short form, keeps every section heading other files cite, and keeps verbatim each zone-registry clause the original carried. This file is the complete text, relocated verbatim so that sessions which never need the detail do not pay for it on every turn. It loads automatically whenever `moai-mcp-tools.md` is read; read it directly whenever the digest points here. For anything the digest does not restate, this file is authoritative.
