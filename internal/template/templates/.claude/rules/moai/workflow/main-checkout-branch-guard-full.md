---
description: "Full text of main-checkout-branch-guard.md — the always-loaded file is a digest; this companion carries the complete doctrine verbatim"
paths: "**/main-checkout-branch-guard.md"
---

> **Full-text companion of `main-checkout-branch-guard.md`.** The always-loaded `main-checkout-branch-guard.md` is a digest: it restates every binding rule in short form, keeps every section heading other files cite, and keeps verbatim each zone-registry clause the original carried. This file is the complete text, relocated verbatim so that sessions which never need the detail do not pay for it on every turn. It loads automatically whenever `main-checkout-branch-guard.md` is read; read it directly whenever the digest points here. For anything the digest does not restate, this file is authoritative.
